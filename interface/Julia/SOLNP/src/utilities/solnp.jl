...
function wrapper_cost(ptr::Ptr{Float64}, n::Int64, result::Ptr{Float64}, n_res::Int64, _solnp_cost::Function)
...
function checkAndWrapFunction(cost, grad, hess)
    # cost
    if cost !== nothing
        cost_for_c = @cfunction((ptr::Ptr{Float64}, result::Ptr{Float64}) -> wrapper_cost(ptr, _solnp_np, result, _solnp_nc + 1, _solnp_cost_for_c), Cvoid, (Ptr{Cdouble}, Ptr{Cdouble}))
    else
