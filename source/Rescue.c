...
)
{
    ...
    w->pb->Ipb = (solnp_float*)solnp_malloc(2 * sizeof(solnp_float));
    w->pb->Ipc = (solnp_float*)solnp_malloc(2 * sizeof(solnp_float));
    ...
    memcpy(w->pb->Ipb, w_old->pb->Ipb, 2 * sizeof(solnp_float));
    memcpy(w->pb->Ipc, w_old->pb->Ipc, 2 * sizeof(solnp_float));
    ...
    SOLNP(add_scaled_array)(w->ob->ec, w->p + w->n + w->nic - w->nec, w->nec, -1.0);
