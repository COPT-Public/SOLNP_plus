...
)
{
    ...
    if (w_sub->alp[0] <= 0) {
        ...
        if (w->pb->Ipb[1] == 0) {
            ...
            SOLNP(scale_array)(&w_sub->J->a[w_sub->J->a_row * w_sub->npic], -1.0, w_sub->J->a_row);
            ...
            SOLNP(Ax)(w_sub->b, w_sub->J->a, w->p, w_sub->J->a_row, w_sub->npic);
    ...
    if (w_sub->alp[0] <= 0) {
        ...
        if (w->pb->Ipb[1] == 0) {
            ...
            while (go >= stgs->tol)
            {
                ...
                SOLNP(transpose)(w_sub->J->a_row, w_sub->npic + 1, a_diag_dx, a_diag_dx_T);

                solnp_float* a_diag_dx2_aT = (solnp_float*)solnp_malloc(w_sub->J->a_row * w_sub->J->a_row * sizeof(solnp_float));
                SOLNP(AB)(a_diag_dx2_aT, a_diag_dx, a_diag_dx_T, w_sub->J->a_row, w_sub->npic + 1, w_sub->J->a_row);
                ...
                SOLNP(Ax)(y, a_diag_dx, dx_c, w_sub->J->a_row, w_sub->npic + 1);
                SOLNP(chol)(w_sub->J->a_row, a_diag_dx2_aT);
                SOLNP(solve_lin_sys)(w_sub->J->a_row, 1, a_diag_dx2_aT, y);
                solnp_float* aT = (solnp_float*)solnp_malloc(w_sub->J->a_row * (w_sub->npic + 1) * sizeof(solnp_float));
                SOLNP(transpose)(w_sub->J->a_row, w_sub->npic + 1, w_sub->J->a, aT);
                SOLNP(Ax)(v, aT, y, w_sub->npic + 1, w_sub->J->a_row);
...
(
    ...
    qpsolver(w_sub->J->npic, w->h, g, w_sub->J->a_row, w_sub->J->npic, w_sub->J->a, c, lb, ub, u, y);
    solnp_free(lb);
    solnp_free(ub);
    /*
    else {
    ...
    while (go <= 0 && affit < 20) {
        ...
        if (sys == 1) {
            ...
            if (w_sub->J->a_row > 0) {
                ...
                SOLNP(transpose)(w_sub->J->a_row, w_sub->J->npic, w_sub->J->a, inv_H_aT);
                SOLNP(solve_lin_sys)(w_sub->J->npic, w_sub->J->a_row, L, inv_H_aT);

                solnp_float* a_inv_H_aT = (solnp_float*)solnp_malloc((w_sub->J->a_row * w_sub->J->a_row) * sizeof(solnp_float));
                SOLNP(AB)(a_inv_H_aT, w_sub->J->a, inv_H_aT, w_sub->J->a_row, w_sub->J->npic, w_sub->J->a_row);

                SOLNP(chol)(w_sub->J->a_row, a_inv_H_aT);
                SOLNP(Ax)(y, w_sub->J->a, u, w_sub->J->a_row, w_sub->J->npic); // au = a  H^-1 u
                SOLNP(scale_array)(y, -1.0, w_sub->J->a_row);

                SOLNP(solve_lin_sys)(w_sub->J->a_row, 1, a_inv_H_aT, y); // a_inv_H_aT * y = a*H^-1*u

                solnp_float* inv_H_aT_y = (solnp_float*)solnp_malloc(w_sub->J->npic * sizeof(solnp_float));
                SOLNP(Ax)(inv_H_aT_y, inv_H_aT, y, w_sub->J->npic, w_sub->J->a_row);
            ...
            SOLNP(set_as_scaled_array)(u, u, -1., w_sub->npic);
...
)
{
    ...
    SOLNP(add_scaled_array)(p0, g, w->n + w->nic, -stepsize);
    return;
}
...
(
    ...
    */
    ...
    while ((lam_ub - lam_lb) >= tol && tag){
        ...
        SOLNP(add_scaled_array)(q_lam_G, G, 4, lam);
        
        memcpy(alpha_mval, c, 2 * sizeof(solnp_float));
        SOLNP(chol)(2, q_lam_G);
        SOLNP(solve_lin_sys)(2, 1, q_lam_G, alpha_mval);
